...
class Invariant(StrEnum):
    ...
    R4_LOOPBACK = "R4"  # CAN reach the env server + a local LLM via the sanctioned path
...
CONTRACT: tuple[InvariantSpec, ...] = (
    ...
    InvariantSpec(
        ...
        "allow",
        "reach the env server + a local LLM via the sanctioned loopback path "
        "(direct TCP, or a bridged socket file under a network namespace)",
    ),
