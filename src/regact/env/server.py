...
class EnvServer:
    ...
    def total_action_count(self, game_id: str) -> int:
        """Env steps taken on the game so far, across all its instances (0 before it is
        started). The loop polls this to surface the agent's move budget; a plain int
        read, safe across threads."""
        session = self._sessions.get(game_id)
        return session.total_action_count if session is not None else 0
