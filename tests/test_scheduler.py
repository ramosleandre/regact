...
def _config(workers: int, lifecycle: Lifecycle) -> RunConfig:
    return RunConfig(
...
async def test_sequential_runs_in_order() -> None:
    ...
    cfg = _config(1, Lifecycle.MULTI_INSTANCE)
...
async def test_parallel_runs_all_tasks() -> None:
    ...
    cfg = _config(3, Lifecycle.MULTI_INSTANCE)
...
def test_validate_rejects_duplicate_game_in_single_instance_parallel() -> None:
    cfg = _config(2, Lifecycle.SINGLE_INSTANCE)
...
def test_validate_allows_duplicates_when_sequential() -> None:
    cfg = _config(1, Lifecycle.SINGLE_INSTANCE)
